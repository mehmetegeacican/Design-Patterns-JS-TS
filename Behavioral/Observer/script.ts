...
class Store implements IStore {
  ...
  //Method
  addSubscriberCustomer(newSubscriber: Customer): void {
    if (this.subscribers.filter((item:Customer) => item.name === newSubscriber.name).length <= 0) {
  ...
  removeSubscriber(oldSubscriber:Customer): void {
    if(this.subscribers.filter((item:Customer) => item.name === oldSubscriber.name).length > 0){
